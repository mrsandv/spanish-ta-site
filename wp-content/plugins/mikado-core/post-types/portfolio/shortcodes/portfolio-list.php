...
class PortfolioList implements Lib\ShortcodeInterface {
            ...
            vc_map( array(
                ...
                "params" => array(
                    ...
                    array(
                        ...
                        "value" => "",
                        "description" =>  __("Please insert top margin for load more button", 'mkd_core'),
    ...
    public function render($atts, $content = null) {
        ...
        if (isset($mkd_options['portfolio_mkd_like'])) {
            $portfolio_mkd_like = $mkd_options['portfolio_mkd_like'];
        }
        ...
        if (isset($mkd_options['paspartu_portfolio_full_width']) && $mkd_options['paspartu_portfolio_full_width'] =='yes' ){
        ...
        if ($item_border == "yes") {
        	if ($type == 'standard_no_space' || $type == 'text_on_hover_image_no_space' || $type == 'text_before_hover_no_space') {
        		if ($item_border_color !== '') {
					...
					$item_border_width_value = 1;
        			if ($item_border_width !== '') {
        				$item_border_width_px = (strstr($item_border_width, 'px', true)) ? $item_border_width : $item_border_width . "px";		
						$item_border_width_value = (strstr($item_border_width_px, 'px', true));
        			}
					$article_style[] = "margin: -" .($item_border_width_value/2) ."px";
					$portfolio_box_style[] = "margin: 0;";
        ...
        if ($overlay_background_color != '' || $gradient_position != '') {
            if ($hover_type == "gradient_hover") {
                if (substr($overlay_background_color, 0, 3) === "rgba") { // if rgba is set, portfolio uses default color
                    ...
                    $rgb = mkd_hex2rgb($overlay_background_color);
        ...
        if (($box_border == "yes" || $box_background_color != "" || $info_box_padding != "") && ($type == 'standard' || $type == 'standard_no_space' || $type == 'masonry_with_space')) {
            ...
            if ($box_border == "yes") {
				...
				$portfolio_box_style[] = 'overflow: visible';
            }
        ...
        if ($type != 'masonry') {
            ...
            if ($filter == "yes") {
                ...
                if ($disable_filter_title != "yes") {
                    $html .= "<li class='filter_title'><span>" . __('Sort Portfolio:', 'mkd_core') . "</span></li>";
                }
                if ($type == 'masonry_with_space' || $type == 'masonry_with_space_without_description') {
                    $html .= "<li class='filter' data-filter='*'><span>" . __('All', 'mkd_core') . "</span></li>";
                } else {
                    $html .= "<li class='filter' data-filter='all'><span>" . __('All', 'mkd_core') . "</span></li>";
                }
            ...
            if (have_posts()) : while (have_posts()) : the_post();
                    ...
                    if ($overlay_background_color != '' || $gradient_position != '') {
                        if ($hover_type == "gradient_hover") {
                            if (substr($overlay_background_color_custom, 0, 3) === "rgba") { // if rgba is set, portfolio uses default color
                                ...
                                $rgb = mkd_hex2rgb($overlay_background_color_custom);
                ...
                $category_html = "";
                /* $category_html .= '<span>'. __('In ', 'mkd_core') .'</span>'; */
                $k = 1;
                foreach ($terms as $term) {
                ...
                if ($type == "standard" || $type == "standard_no_space" || $type == "masonry_with_space") {
                    $html .= "<div class='portfolio_description " . $portfolio_description_class . "' ".mkd_get_inline_style(implode('; ', $portfolio_box_style)).">";
            ...
            endwhile;
                ...
                ?>
                <p><?php _e('Sorry, no posts matched your criteria.', 'mkd_core'); ?></p>
            <?php
            endif;
            ...
            if (get_next_posts_link()) {
                if ($show_load_more == "yes" || $show_load_more == "") {
                    $html .= '<div class="portfolio_paging" ' . $show_more_style . '><span data-rel="' . $wp_query->max_num_pages . '" class="load_more">' . get_next_posts_link(__('Show more', 'mkd_core')) . '</span></div>';
                    $html .= '<div class="portfolio_paging_loading"><a href="javascript: void(0)" class="qbutton">' . __('Loading...', 'mkd_core') . '</a></div>';
                }
            }
