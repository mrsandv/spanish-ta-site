...
$args = array_merge($args, $mkdIconCollections->getShortcodeParams('content_menu_'));
...
$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'vc_row wpb_row section '. ( $this->settings('base')==='vc_row_inner' ? 'vc_inner ' : '' ) . $el_class, $this->settings['base'], $atts );
...
$header_style_data = '';
if($header_style != ""){
	$header_style_data =  'data-mkd_header_style="'.$header_style.'"';
}
...
$anchor_id = "";
if($anchor != ""){
	$anchor_id = ' data-mkd_id="#'.$anchor.'"';
}

$menu_title = "";
if($content_menu_title != ""){
	$menu_title = ' data-mkd_title="'.$content_menu_title.'"';
}

$menu_icon = '';
if ($icon_pack !== '' && $in_content_menu !== '') {

	$item_collection_icon_obj = $mkdIconCollections->getIconCollection($icon_pack);
...
if($row_type == 'row') {
	...
	if($logo_in_content_menu == 'logo_in_content_menu'){
		...
		if(isset($mkd_options['content_menu_logo_image']) && $mkd_options['content_menu_logo_image'] != ""){
			$logo_image = $mkd_options['content_menu_logo_image'];
		}else if(isset($mkd_options['logo_image']) && $mkd_options['logo_image'] != ""){
			$logo_image = $mkd_options['logo_image'];
		}
	...
	if($custom_widget_area !== ''){
		...
		$output .= mkd_get_dynamic_sidebar($custom_widget_area);
		$output .= '</div></div></div>';
	}
