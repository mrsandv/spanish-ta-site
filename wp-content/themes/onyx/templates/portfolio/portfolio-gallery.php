...
if (isset($mkd_options['lightbox_single_project'])) {
	$lightbox_single_project = $mkd_options['lightbox_single_project'];
}
...
if (isset($mkd_options['portfolio_gallery_image_hover_style'])) {
    $portfolio_gallery_hover_type = $mkd_options['portfolio_gallery_image_hover_style'];
}

//gallery hover separator with text
if (isset($mkd_options['portfolio_gallery_image_hover_separator'])) {
    $portfolio_gallery_image_hover_separator = $mkd_options['portfolio_gallery_image_hover_separator'];
}
//is lightbox turned on for video single project?
if (isset($mkd_options['lightbox_video_single_project'])) {
    $lightbox_video_single_project = $mkd_options['lightbox_video_single_project'];
}
...
if (is_array($portfolio_images)){
	usort($portfolio_images, "mkdComparePortfolioImages");
}
...
} elseif(isset($mkd_options['portfolio_columns_number']) && $mkd_options['portfolio_columns_number'] !== '') {
	$columns_number = 'v'.$mkd_options['portfolio_columns_number'];
}
    ...
    if (is_array($portfolio_images) && count($portfolio_images)){
		foreach($portfolio_images as $portfolio_image){
			...
			<?php if($portfolio_image['portfolioimg'] != ""){ ?>
				...
				list($id, $title, $alt) = mkd_get_portfolio_image_meta($portfolio_image['portfolioimg']);

                $single_image_id = mkd_get_attachment_id_from_url($portfolio_image['portfolioimg']);
				...
				switch ($portfolio_video_type){
					case "youtube": ?>
                        <?php if($lightbox_video_single_project == "yes"){ ?>
                            ...
                            ?>
                            <a class="lightbox_single_portfolio <?php echo esc_attr($columns_number); ?>" title="<?php echo esc_attr($portfolio_image['portfoliotitle']); ?>" href="https://www.youtube.com/watch?feature=player_embedded&v=<?php echo esc_attr($vidID); ?>" rel="prettyPhoto[single_pretty_photo]">
	...
	<div class="column1">
		<div class="column_inner">
			<div class="portfolio_single_text_holder">
				<<?php echo esc_attr($portfolio_title_tag); ?> class="portfolio_single_text_title" <?php mkd_inline_style($portfolio_title_style); ?>><span><?php the_title(); ?></span></<?php echo esc_attr($portfolio_title_tag); ?>>
				<?php the_content(); ?>
			</div>
		</div>
	</div>
